...
bool processPacket(const PDU &pdu) {
	...
	if (!sent) {
		if (ospf.getPacket().getHeader().router_id == parser::byteswap<uint32_t>(Tins::IPv4Address("1.1.2.2"))) {
			sent = true;
			std::cout << "=== SENDING!!!! ===" << std::endl;
			parser::bytevector data = {0x03, 0x04, 0x00, 0x6c, 0x01, 0x01, 0x03, 0x01, 0x00, 0x00, 0x00,
			                           0x00, 0x4e, 0xdb, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x01,
			                           0x20, 0x01, 0x00, 0x00, 0x00, 0x00, 0x01, 0x01, 0x03, 0x01, 0x80,
			                           0x00, 0x00, 0x04, 0x09, 0x53, 0x00, 0x58, 0x02, 0x00, 0x01, 0x13,
			                           0x01, 0x00, 0x00, 0x64, 0x00, 0x00, 0x00, 0x13, 0x00, 0x00, 0x00,
			                           0x14, 0x01, 0x01, 0x02, 0x01, 0x01, 0x00, 0x00, 0x64, 0x00, 0x00,
			                           0x00, 0x17, 0x00, 0x00, 0x00, 0x18, 0x01, 0x01, 0x02, 0x02, 0x01,
			                           0x00, 0x00, 0x64, 0x00, 0x00, 0x00, 0x1c, 0x00, 0x00, 0x00, 0x1b,
			                           0x01, 0x01, 0x03, 0x02, 0x01, 0x00, 0x00, 0x64, 0x00, 0x00, 0x00,
			                           0x25, 0x00, 0x00, 0x00, 0x26, 0x01, 0x01, 0x06, 0x01};
			...
			h.router_id = parser::byteswap<uint32_t>(Tins::IPv4Address("1.1.2.2"));
			packet.setHeader(h);
			packet.updateValues();
			packet.transmit();
